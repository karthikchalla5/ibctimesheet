...
(function ($, undefined) {
    ...
    var Ace_File_Input = function (element, settings) {
        ...
        this.$element.on('change.ace_inner_call', function (e, ace_inner_call) {
            if (ace_inner_call === true)
                return;//this change event is called from above drop event
    ...
    Ace_File_Input.prototype.apply_settings = function () {
        ...
        if (remove_btn)
            this.$label.next('a').on(ace.click_event, function () {
                ...
                var ret = true;
                if (self.settings.before_remove)
                    ret = self.settings.before_remove.call(self.element);
                if (!ret)
                    return false;
    ...
    Ace_File_Input.prototype.show_file_list = function ($files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (!this.well_style)
                this.$label.find('.file-name').attr({'data-title': filename}).find('[class*="icon-"]').attr('class', fileIcon);
            else {
                ...
                if (can_preview) {
                    ...
                    $.when(preview_image.call(this, files[i])).fail(function (result) {
                        ...
                        if (self.settings.preview_error)
                            self.settings.preview_error.call(self, filename, result.code);
                    });
                }
            }
    ...
    var enable_drop_functionality = function () {
        ...
        $(dropbox).on('dragenter', function (e) {
            ...
            var ret = true;
            if (self.settings.before_change)
                ret = self.settings.before_change.call(self.element, files, true);//true means files have been dropped
            if (!ret || ret.length == 0) {
                return false;
            }

            //user can return a modified File Array as result
            if (ret instanceof Array || (hasFileList && ret instanceof FileList))
                files = ret;
    ...
    var handle_on_change = function () {
        ...
        if (this.settings.before_change)
            ret = this.settings.before_change.call(this.element, this.element.files || [this.element.value]/*make it an array*/, false);//false means files have been selected, not dropped
        if (!ret || ret.length == 0) {
            if (!this.$element.data('ace_input_files'))
                this.reset_input_field();//if nothing selected before, reset because of the newly unacceptable (ret=false||length=0) selection
            return false;
        }
    ...
    var preview_image = function (file) {
        ...
        reader.onload = function (e) {
            ...
            $(img).one('load', function () {
                ...
                var size = 50;
                if (self.settings.thumbnail == 'large')
                    size = 150;
                else if (self.settings.thumbnail == 'fit')
                    size = $span.width();
    ...
    var get_thumbnail = function (img, size, type) {
        ...
        if (!(/^data\:image\/(png|jpe?g|gif);base64,[0-9A-Za-z\+\/\=]+$/.test(dataURL)))
            dataURL = null;
        if (!dataURL)
            return null;
    ...
    $.fn.ace_file_input = function (option, value) {
        ...
        var $set = this.each(function () {
            ...
            if (!data)
                $this.data('ace_file_input', (data = new Ace_File_Input(this, options)));
            if (typeof option === 'string')
                retval = data[option](value);
        });
...
(function ($, undefined) {
    $.fn.ace_wysiwyg = function ($options, undefined) {
        ...
        this.each(function () {
            ...
            toolbar.find('.dropdown-menu input:not([type=file])').on(ace.click_event, function () {
                return false
            })
                    .on('change', function () {
