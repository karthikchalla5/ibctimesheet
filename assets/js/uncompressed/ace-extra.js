...
ace.settings = {
    ...
    navbar_fixed: function (fix) {
        ...
        if (fix) {
            if (!ace.hasClass(navbar, 'navbar-fixed-top'))
    ...
    breadcrumbs_fixed: function (fix) {
        ...
        if (fix) {
            if (!ace.hasClass(breadcrumbs, 'breadcrumbs-fixed'))
    ...
    main_container_fixed: function (inside) {
        ...
        if (inside) {
            if (!ace.hasClass(main_container, 'container'))
                ace.addClass(main_container, 'container');
            if (!ace.hasClass(navbar_container, 'container'))
...
ace.settings.check = function (item, val) {
    if (!ace.settings.exists(item, val))
        return;//no such setting specified
